...
const LoginPage = () =>
{
  ...
  const handleSubmit = async (e) =>
  {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5000/api/syndic/login', {
        email: formData.email,
        password: formData.password,
      });
      console.log('Syndic Logged successfully:', response.data);

      // Dispatch the action to store the login data in Redux
        ...
        <div className="lg:w-1/2 xl:max-w-screen-sm">
          <div className="py-12 bg-indigo-100 lg:bg-white flex justify-center lg:justify-start lg:px-12">
            <div className="cursor-pointer flex items-center">
              ...
              <div className="text-2xl text-indigo-800 tracking-wide ml-2 font-semibold">DigiSynd</div>
            </div>
          </div>
          ...
          <div className="mt-10 px-12 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
            ...
            <div className="mt-12">
              <form onSubmit={handleSubmit}>
                ...
                <div className="mt-10">
                  <button className="bg-indigo-500 text-gray-100 p-4 w-full rounded-full tracking-wide font-semibold font-display focus:outline-none focus:shadow-outline hover:bg-indigo-600 shadow-lg">Log In</button>
                </div>
              </form>

              <div className="mt-12 text-sm font-display font-semibold text-gray-700 text-center">
                This platforme is secured <a className="cursor-pointer text-indigo-600 hover:text-indigo-800">Secured</a>
              </div>
            </div>
          </div>
        </div>
